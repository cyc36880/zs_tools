...
int main(void)
{
	...
	/*
	 * The list has only 14 elements from the `current` pointer now.
	...
	*/
	...
	assert(cc_list_cursor_remove(cursor, 14, 1) ==
			CC_LIST_CURSOR_MOVE_OUT_OF_RANGE);
	assert(cc_list_cursor_remove(cursor, 13, 2) ==
			CC_LIST_CURSOR_MOVE_OUT_OF_RANGE);
	assert(!cc_list_cursor_remove(cursor, 13, 1));

	/*
	 * The list has only 13 elements from the `current` pointer now.
	...
	*/
	...
	assert(cc_list_cursor_insert_before(cursor, 16, (void *)99) ==
			CC_LIST_CURSOR_MOVE_OUT_OF_RANGE);
	assert(cc_list_cursor_insert_before(cursor, 15, (void *)99) ==
			CC_LIST_CURSOR_MOVE_OUT_OF_RANGE);
	assert(cc_list_cursor_insert_before(cursor, 14, (void *)99) ==
			CC_LIST_CURSOR_MOVE_OUT_OF_RANGE);
	assert(!cc_list_cursor_insert_before(cursor, 13, (void *)99));
	assert(cc_list_cursor_get(cursor, 12, 3, (void **)buffer) ==
			CC_LIST_CURSOR_GET_OUT_OF_RANGE);
	...
	*/
	...
	assert(cc_list_cursor_get(cursor, -1, 15, (void **)buffer) ==
			CC_LIST_CURSOR_GET_OUT_OF_RANGE);
	...
	assert(cc_list_cursor_remove(cursor, 0, 1) ==
			CC_LIST_CURSOR_REMOVING_CURRENT);
	assert(cc_list_size(cursor->list) == 1);
